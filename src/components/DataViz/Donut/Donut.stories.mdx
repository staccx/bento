...
## Donut with progress 100% with Slider for progress
<Preview>
  <Story name="100%">
   {() => {
       ...
       <Slider min={0} name={"progress"} max={1} step={0.01} value={1} onChange={e => setProgress(e.target.value)} label="Set Progress"/>
