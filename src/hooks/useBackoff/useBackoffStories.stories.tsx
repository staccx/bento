...
const helperFunction = (shouldFail) => new Promise(async (resolve, reject) => {
...
const BaseTestComponent = args => {
  ...
  const { result, state, error, retries, reset, nextRetry } = useBackoff(() => helperFunction(fail), {
    ...args
  });

  return (
    <div>
      <h3>This component is a reusable component to allow easier maintenance. For examples see the code</h3>
      <div>
...
export const UseBackoffSuccess = args => {
  return <BaseTestComponent timeBeforeSuccess={10000}
                            time={1000}
                            maxTime={5000}
                            depth={5}
...
export const UseBackoffFailed = args => {
  return <BaseTestComponent timeBeforeSuccess={100000}
                            time={100}
                            maxTime={1000}
                            depth={7}
                            strategy={BackoffStrategy.Exponential} />;
...
export const UseBackoffFixed = args => {
  return <BaseTestComponent timeBeforeSuccess={5000}
                            time={100}
                            maxTime={100}
                            depth={7}
