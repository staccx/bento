...
<Preview>
  <Story name="Default">
   ...
   return (
        <div>
       <div>State: {state}</div>
  {state === "idle" && <Button onClick={submit}>Submit</Button>}
      {state === "requesting" && <Loading idleTime={0}/>}
      {state === "success" && <span>Result was {result}</span>}
      {state === "success" && <Button onClick={retry}>Retry</Button>}
      {state === "error" && <span>Error is {error}</span>}
        </div>
      )
   }}
  </Story>
