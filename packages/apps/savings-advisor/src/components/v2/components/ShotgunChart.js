...
class ShotgunChart extends Component {
  ...
  componentDidMount() {
    ...
    const { forecast, backtest, isForecast } = this.props
  ...
  componentDidUpdate() {
    const { backtest, forecast, isForecast } = this.props
  ...
  render() {
    ...
    const { forecast, backtest } = this.props // eslint-disable-line no-unused-vars
    return (
      <canvas
        ref={this.chartRef}
